...
public class IPListGetter extends Thread{
    ...
    public void run() {
        try {
            ...
            if (connection.getResponseCode() == 200) {
                ...
                ArrayList<IPListGetter.PingThread> pingThreads = new ArrayList<>();
                for (String ip : ipList)
                    pingThreads.add(new IPListGetter.PingThread(ip));
                for (IPListGetter.PingThread pingThread : pingThreads)
                    pingThread.start();
    ...
    // ping一个特定的地址，然后记录rtt
    static class PingThread extends Thread implements Comparable<IPListGetter.PingThread> {
        long rtt;
        String ip;
        boolean finished;
        ...
        @Override
        public int compareTo(IPListGetter.PingThread pingThread) {
